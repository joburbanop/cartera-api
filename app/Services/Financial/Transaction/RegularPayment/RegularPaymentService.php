...
class RegularPaymentService
{
    ...
    public function registerRegularPayment(CreateTransactionDTO $dto): Transaction
    {
        ...
        return DB::transaction(function () use ($dto) {
            ...
            if ($this->hasExtraordinaryOption($dto)) {
                ...
                $prior = $toProcess
                    ->takeWhile(fn (AmortizationInstallment $row) => (int) $row->id !== (int) $plan->id)
                    ->values();
            ...
            $transaction = Transaction::create([
                ...
                'payment_option' => filled($dto->paymentOption) ? strtolower(trim((string) $dto->paymentOption)) : null,
            ]);
            ...
            if (bccomp($surplus, '0.00', 2) > 0) {
                ...
                if (in_array($recalculationType, ['reducir_plazo', 'reduce_term', 'abono_capital'], true)) {
                    app(TermReductionService::class)
                        ->recalculateFuture($contract, $plan->fresh());
                } elseif ($recalculationType === 'reducir_cuota' || $recalculationType === 'reduce_quota') {
                    app(PaymentReductionService::class)
                        ->recalculateFuture($contract, $plan->fresh());
                }
    ...
    private function hasExtraordinaryOption(CreateTransactionDTO $dto): bool
    {
        ...
        foreach ($candidates as $candidate) {
            ...
            if (in_array($normalized, ['reducir_plazo', 'reducir_cuota', 'adelantar_cuotas', 'abono_capital'], true)) {
                return true;
            }
        }
